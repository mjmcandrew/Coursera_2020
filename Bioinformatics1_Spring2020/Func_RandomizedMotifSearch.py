def RandomizedMotifSearch(Dna, k, t):
    ...
    #Sets the list 'BestMotifs' equal to 'random_motifs' selected.
    while True:
        #Sets a 'while' loop that will terminate when the score no longer
        #improves.
        Profile = MotifProfileWithPseudocounts(random_motifs)
        #Obtains profile of random_motifs.
        random_motifs = MotifsFinder(Profile, Dna)
        #Resets random_motifs based on the profile most probable kmer in
        #each string.
        if MotifScore(random_motifs) < MotifScore(BestMotifs):
            BestMotifs = random_motifs
            #If the score of the random motifs is better than the score
            #of the previous set of random motifs, 'BestMotifs' is replaced
            #by the current set of random motifs.
        else:
            return BestMotifs
            #When the score does not improve, the motifs are returned.
