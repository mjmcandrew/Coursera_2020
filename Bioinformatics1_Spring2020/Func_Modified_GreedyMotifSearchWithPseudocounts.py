def GreedyMotifSearchWithPseudocounts(Dna, k, t, pseudocount):
    ...
    for index in range(n-k+1):
        ...
        for string in range(1, t):
            ...
            motif_profile = MotifProfileWithPseudocounts(Motifs[0:string], pseudocount)
        ...
        if MotifScore(Motifs, pseudocount) < MotifScore(BestMotifs, pseudocount):
...
def MotifProfileWithPseudocounts(Motifs, pseudocount):
    profile = {}
    ...
    for symbol in "ACGT":
        ...
        for index in range(kmer_length):
             profile[symbol].append(pseudocount)
    ...
    counts = MotifCountWithPseudocounts(Motifs, pseudocount)
    ...
    for nucleotide in counts:
        ...
        for index in range(kmer_length):
            ...
            frequency = value / float((motif_number + (4 * pseudocount)))
...
def MotifCountWithPseudocounts(Motifs, pseudocount):
    motif_number = len(Motifs)
    ...
    for symbol in "ACGT":
        ...
        for index in range(motif_length):
             count[symbol].append(pseudocount)
...
def MotifConsensus(Motifs, pseudocount):
    kmer_length = len(Motifs[0])
    #Determines the kmer_length based on the length of the first kmer.
    counts = MotifCountWithPseudocounts(Motifs, pseudocount)
    #Runs MotifCount on the motifs being analyzed to determine the nucleotide
...
def MotifScore(Motifs, pseudocount):
    motif_score = 0
    #Sets motif score equal to 0.
    kmer_length = len(Motifs[0])
    #Determines the kmer_length based on the length of the first kmer.
    motif_number = len(Motifs)
    consensus = MotifConsensus(Motifs, pseudocount)
